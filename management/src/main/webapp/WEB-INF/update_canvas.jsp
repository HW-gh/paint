...
<div class="container">
    ...
    <form class="form-horizontal" action="/back/updateCanvas.do" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <select id="categoryId" name="categoryId" class="col-sm-2 form-control" style="width: auto">
                <c:forEach items="${categorys}" var="temp">
                    <option value="${temp.id}" <c:if test="${temp.id==canvas.categoryId}" >selected="selected"</c:if>>${temp.name}</option>
                </c:forEach>
            </select>
        </div>
        <div class="form-group">
